...
type KubeControllersConfigurationLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.KubeControllersConfiguration, err error)
...
// List lists all KubeControllersConfigurations in the indexer.
func (s *kubeControllersConfigurationLister) List(selector labels.Selector) (ret []*v1alpha1.KubeControllersConfiguration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.KubeControllersConfiguration))
	})
	return ret, err
}
...
type KubeControllersConfigurationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.KubeControllersConfiguration, error)
...
// List lists all KubeControllersConfigurations in the indexer for a given namespace.
func (s kubeControllersConfigurationNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.KubeControllersConfiguration, err error) {
...
// Get retrieves the KubeControllersConfiguration from the indexer for a given namespace and name.
func (s kubeControllersConfigurationNamespaceLister) Get(name string) (*v1alpha1.KubeControllersConfiguration, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("kubecontrollersconfiguration"), name)
	}
	return obj.(*v1alpha1.KubeControllersConfiguration), nil
}
